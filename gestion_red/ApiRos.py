...
def main():
    resultados =[]
    s = None
    router_address = sys.argv[1] #"10.95.12.1"#"fe80::de2c:6eff:feab:7704%2"
    user = sys.argv[2]#"solit"
    passw = sys.argv[3]#"Pa238388$"
    comando =sys.argv[4]
    secure = False
    port = 8728
    ...
    s = open_socket(router_address, port, secure)
    if s is None:
        print(f"Could not open socket for router {router_address}")
    

    apiros = ApiRos(s)
    if not apiros.login(user, passw):
        print(f"Failed to login to router {router_address}")
    ...
    resultado = apiros.talk([comando])
    resultados.append(((router_address), resultado))
...
def datos():
    ...
    if __name__ == "__main__":
            resultados =main()
            serialized_resultado = {}
            for router_address, resultado in resultados:
                    router_data = {
                        "resultado": resultado,
                        
                    }
                    serialized_resultado[router_address]= router_data

            return serialized_resultado
